    ...
    <div className="well">
        <div className="card bg-transparent norder-0 shadow-sm mt-3">
            ...
            <div className="card-body">
                ...
                { props.config.colors.map( (color, key) => <span key={key} className={`filter-color badge rounded-circle p-1 ${ props.filter.color === color.value ? 'color-active' : '' }`} onClick={ () => props.updateFilter({ color: color.value }) } style={{ backgroundColor: color.value }}> { color.value.slice(0, 1).toUpperCase() } </span> ) }                    
                </div>
                <h5 className="card-title text-muted mt-4">Size</h5>
                <ul className="nav nav-pills filter-size">
                    { props.config.sizes.map( (size, key) => (
                        <li className="nav-item" key={key} onClick={ () => props.updateFilter({ size: size.value }) }>
                    ...
                    { props.config.departments.map ( ( dept, k ) => (
                        <button key={k} type="button" className="list-group-item list-group-item-action border-0">
                            <div className="form-check">
                                <input className="form-check-input" onClick={ (e) => props.updateDepartmentFilter(dept.name, e.target.checked) } type="checkbox" id="defaultCheck1"/>
